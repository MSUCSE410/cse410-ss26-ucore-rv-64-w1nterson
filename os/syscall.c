...
uint64 sys_task_info(uint64 ti_va)
{
    ...
    if (p->started && now >= p->start_cycle) {
...
uint64 sys_mmap(uint64 start, uint64 len, int port, int flag, int fd)
{
    ...
    // at least one of R/W/X must be set
    if ((port & 0x7) == 0)
        return -1;
    ...
    // Build the PTE permission bits.
    // port bit 0 = readable  -> PTE_R
    // port bit 1 = writable  -> PTE_W
    // port bit 2 = executable-> PTE_X
    // Always add PTE_U so user mode can access it.
    int perm = PTE_U;
    ...
    // Check that no page in [start, start+len) is already mapped
    ...
    // Map each page individually (kalloc does not guarantee contiguous frames)
    for (va = start; va < end; va += PGSIZE) {
        void *mem = kalloc();
        if (mem == 0) {
            // Out of memory — spec says we don't need to recover
            return -1;
        }
        memset(mem, 0, PGSIZE);
    ...
    // Update max_page if this mapping extends beyond the current high watermark
...
uint64 sys_munmap(uint64 start, uint64 len)
{
    ...
    // Verify every page in [start, start+len) is currently mapped
